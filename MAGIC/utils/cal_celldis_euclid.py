...
for i in range(len(X)):
    ...
    for j in range(len(X)):
        if(i>j):
            ...
            d3=euclid(X3[i],X3[j])
            dis3[i][j]=d3            
np.savetxt('celldis_no_norm_euc.txt',dis)
np.savetxt('celldis_cp10k_euc.txt',dis1)
np.savetxt('celldis_log_cp10k_euc.txt',dis2)
np.savetxt('celldis_sqrt_cp10k_euc.txt',dis3)
